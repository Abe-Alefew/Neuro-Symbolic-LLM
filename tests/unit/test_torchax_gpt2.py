"""Tests for substrate.torchax_gpt2 -- run entirely against a local
random-init GPT2Config, no network needed. What's verified here does not
depend on which weight values are loaded, only on TorchAX's dispatch
behaving consistently for the GPT-2 architecture -- see the module
docstring for what still needs re-confirming against a real checkpoint."""
...
from substrate.torchax_gpt2 import functional_gpt2
...
class TestFunctionalCallCorrectness:
    ...
    def test_numerically_matches_plain_pytorch(self):
        ...
        # same order of magnitude as this project's other fidelity checks
...
class TestFreezing:
    ...
    def test_frozen_params_receive_no_gradient_through_functional_call(self):
        ...
        # Nothing requires grad here (no residual params in this test), so
        # this just confirms the frozen path itself raises no error and
        # produces no grad-carrying leaves -- the interception+gradient
        # case belongs to Person 2's module, not this one.
...
class TestHookSurvivesFunctionalCall:
    def test_hook_fires_and_can_replace_output(self):
        ...
        def hook(module, inp, output):
            ...
            return output  # identity for this test; replacement is Person 2's concern
