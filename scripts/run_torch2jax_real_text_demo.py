#!/usr/bin/env python
"""End-to-end real-text demo for the frozen JAX substrate (Segmented torch2jax Mechanism M2).
...
Usage:
    python scripts/run_torch2jax_real_text_demo.py
    python scripts/run_torch2jax_real_text_demo.py --model EleutherAI/pythia-70m
    python scripts/run_torch2jax_real_text_demo.py --text data/wiki.txt --layers 0,5,11
    python scripts/run_torch2jax_real_text_demo.py --steer 2.0
"""
...
def main() -> int:
    ...
    section("3. LOADING FROZEN SUBSTRATE (SEGMENTED TORCH2JAX)")
    print(f"Loading     : {args.model} (PyTorch module -> Segmented torch2jax Engine)")
    ...
    plain_sub = FrozenJAXSubstrate(
        torch_model,
        torch_model.config,
        intercept_layers=layers,
    )
    print(f"Segments    : {len(sub._engine.segments)} compiled XLA segments ({', '.join(sub._engine.segment_names)})")

    # ── Segmented forward pass with interception ───────────────────────────
    section("4. FORWARD PASS ACROSS SEGMENTED INTERCEPTION")
