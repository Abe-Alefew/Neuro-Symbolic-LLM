...
5. [Test Suite Documentation](#test-suite-documentation)
   - [10-Step Sequential Validation Strategy](#10-step-sequential-validation-strategy)
...
| **10-Step Validation Strategy** | **Completed** | Full test suite for both GPT-2 (`test_validation_gpt.py`) and Pythia (`test_validation_pythia.py`). |
...
### 10-Step Sequential Validation Strategy
The test modules `tests/frozenllm/unit/test_validation_gpt.py` and `tests/frozenllm/unit/test_validation_pythia.py` implement an end-to-end verification strategy:

```text
Test 1  — Base PyTorch Reference (Establishes ground truth logits, loss, and hidden states)
Test 2  — Functional Model Parity (Verifies TorchAX substrate matches PyTorch within 1e-3)
Test 3  — Identity Interception (Proves identity modify_fn matches baseline logits and loss)
Test 4  — Hidden-State Transparency (Verifies intermediates[l] captures pristine block output)
Test 5  — Two Simultaneous Interception Points (Verifies simultaneous mid/late layer cuts)
Test 6  — Non-Identity Modification (Steering alters downstream logits while pristine cache matches)
Test 7  — Frozen Parameter Gradient Invariant (Verifies all theta_0 parameters have requires_grad=False)
Test 8  — Hidden-State Gradient Availability (Verifies dL/dh_l != 0 downstream propagation)
Test 9  — Loss Consistency (Verifies compute_loss is deterministic, finite, and consistent)
Test 10 — Repeated Execution (Verifies 10 repeated runs yield identical logits and params_unchanged)
```
...
|---|---|
| `test_interception.py` | Hook registration lifecycle, context manager cleanup on normal exit and exceptions, pristine caching invariance, tuple return unwrapping/wrapping, multi-layer cascades. |
| `test_substrate.py` | `FrozenSubstrate` initialization, GPT-2 & Pythia architecture detection, parameter freezing invariance, tokenization helpers, causal cross-entropy loss, and memory status. |
...
| `test_architecture.py` | Config and parameter inspection, layer count discovery, and layer index validation. |
| `test_substrate_memory.py` | Memory headroom calculation, CPU diagnostic handling, and automatic batch-size reduction rules. |
| `test_substrate_invalid.py` | Input validation, rejection of out-of-range layer indices, and malformed tensor inputs. |
...
# Run the 10-step validation suites specifically
pytest tests/frozenllm/unit/test_validation_gpt.py -v
pytest tests/frozenllm/unit/test_validation_pythia.py -v
```
