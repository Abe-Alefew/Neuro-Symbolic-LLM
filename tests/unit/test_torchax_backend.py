"""Tests for substrate.torchax_backend. No network needed -- everything
here is pure mechanism, verified with toy tensors and a toy raw-JAX
function standing in for real FabricPC code."""
...
class TestCallJaxDifferentiable:
    """The critical test: does the packaged bridge actually produce
    correct gradients, not just run without error?"""
    ...
    def test_jax_fn_receives_no_torch_syntax_requirement(self):
        # explicitly confirms the whole point of this wrapper: jax_fn can
